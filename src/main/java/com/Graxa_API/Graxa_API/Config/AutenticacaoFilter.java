        ...
        if (username != null && SecurityContextHolder.getContext().getAuthentication() == null) {
            ...
            if (username != null && SecurityContextHolder.getContext().getAuthentication() == null) {
                if (jwtTokenManager.validaTokenSomente(jwtToken, username)) {
                    ...
                    // Lê roles direto do JWT — sem bater no banco
                    String rolesStr = jwtTokenManager.getRolesFromToken(jwtToken);
                    List<GrantedAuthority> authorities = RoleMapper.fromRolesString(rolesStr);

                    UsernamePasswordAuthenticationToken authentication =
                            new UsernamePasswordAuthenticationToken(username, null, authorities); // ← era List.of()
