...
public record RequestBandasShowDto(
        ...
        @Schema(description = "ID do show ao qual as bandas serão adicionadas", example = "1")
        Long showId,

        @NotEmpty(message = "A lista de IDs das bandas não pode estar vazia")
        @Schema(description = "Lista de IDs das bandas que serão adicionadas ao show", example = "[1]")
        List<Long> bandasIds
) {}
